...
def attention_beam_search(
    ...
    # 1. Encoder
    maxlen = features_a.size(1)
    encoder_dim = features_a.size(2)
    ...
    if model.decoder.use_sdpa:
        mask_a = mask_to_bias(mask_a, features_a.dtype)
        mask_v = mask_to_bias(mask_v, features_v.dtype)
    ...
    for i in range(prefix_len, maxlen + 1):
        ...
        if model.decoder.use_sdpa:
            hyps_mask = mask_to_bias(hyps_mask, features_a.dtype)
        # logp: (B*N, vocab)
        logp = model.decoder.forward_one_step(
            memory_a=features_a,
            memory_a_mask=mask_a,
            memory_v=features_v,
            memory_v_mask=mask_v,
            tgt=hyps,
            tgt_mask=hyps_mask,
            cache=cache,
        )
