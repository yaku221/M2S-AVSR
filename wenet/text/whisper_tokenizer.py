...
class WhisperTokenizer(BaseTokenizer):
    def __init__(
        ...
        **kwargs,
        ...
        self.tokenizer = None  # for lazy load
    ...
    def _build_tiktoken(self):
        if self.tokenizer is None:
            ...
            for i in range(self.tokenizer.encoding.n_vocab):
                # decode_single_token_bytes(i) ==> token_bytes
                # str(token_bytes) ==> "b'AA=="
                unit = str(
                ...
                self.t2i[unit] = i  # 188
                self.i2t[i] = unit  # "b'AA=="
            assert len(self.t2i) == len(self.i2t)

    def tokenize(self, line: str) -> Tuple[List[str], List[int]]:  # text ==> token id
        self._build_tiktoken()
        ids = self.tokenizer.encoding.encode(line)
        text = [self.i2t[d] for d in ids]
        return text, ids

    def detokenize(self, ids: List[int]) -> Tuple[str, List[str]]:  # token id ==> text
        self._build_tiktoken()
