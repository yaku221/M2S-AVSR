...
import wenet.dataset.processor_avsr as processor
from wenet.dataset.datapipes_avsr import (WenetRawDatasetSource,
                                     WenetTarShardDatasetSource)
from wenet.text.base_tokenizer import BaseTokenizer
from wenet.utils.file_utils import read_symbol_table

from wenet.dataset.video_load import build_multiview_index


def _has_video(sample):
    return sample.get('video', None) is not None
...
def Dataset_AV(data_type,
    ...
    """
    # audio_data_list_file: data.list or shards.list
    # video_data_list_file: data.list   
    # NOTE: 
    # .pt video data have normalized, float32, and reshape to 96*96 (tools/avi2pt.py)
    ...
    # load audio data 
    if data_type == 'raw':
        dataset = WenetRawDatasetSource(audio_data_list_file,
                                        partition=partition,
        ...
        dataset = WenetTarShardDatasetSource(audio_data_list_file,
                                             partition=partition,
                                             shuffle=list_shuffle,
                                             shuffle_size=list_shuffle_size,
                                             cycle=cycle)
    ...
    video_data_conf = {}
    video_data_conf['dataset_name'] = conf.get('dataset_name', None)
    video_data_conf['video_res'] = conf.get('video_res', 96)
    video_data_conf['video_aug'] = conf.get('video_aug', True)
    video_data_conf['train'] = conf.get('train', False)
    video_data_conf['directions'] = conf.get('directions', 'd1')
    allowed = {"d0", "d1", "d2", "all"}
    if video_data_conf['directions'] not in allowed:
        raise ValueError(f"Invalid directions={video_data_conf['directions']}, expected one of {sorted(allowed)}")  
    video_data_conf['max_speakers'] = conf.get('max_speakers', '5')  

    video_key2lips = build_multiview_index(video_data_list_file, video_data_conf['dataset_name'])
    # import ipdb; ipdb.set_trace()
    video_data_conf['video_key2lips'] = video_key2lips
    dataset = dataset.flatmap_ignore_error(partial(processor.load_video_by_key, **video_data_conf))
    dataset = dataset.filter(_has_video)
    ...
    # add noise
    noise_enable = conf.get('add_noise', False)
    if noise_enable:
        split = conf.get('split', None)
        train_flag = conf.get('train', None)
        split_display = split if split is not None else ('train' if train_flag else 'unknown')
        if train_flag:
            noise_list = conf.get('noise_fn_train', None)
            noise_snr  = conf.get('noise_snr_train', 0)
        elif split == 'cv':
            noise_list = conf.get('noise_fn_dev', None)
            noise_snr  = conf.get('noise_snr_dev', 0)
        else:
            noise_list = conf.get('noise_fn_test', None)
            noise_snr  = conf.get('noise_snr_test', 0)

        noise_prob = conf.get('noise_prob', 0.0)

        if noise_list is not None:
            logging.info(f"Adding noise for {split_display} "
                        f"(prob={noise_prob}, snr={noise_snr} dB, list={noise_list})")
        else:
            logging.info(f"No noise added for {split_display}.")
        ...
        dataset = dataset.map(partial(processor.add_noise, **noise_conf))
    else:
        logging.info("No noise added in audio files.")
    ...
    if batch_type == 'static':
        ...
        dataset = dataset.batch(batch_size, wrapper_class=processor.padding_av)
        ...
        dataset = dataset.bucket_by_sequence_length(
            ...
            wrapper_class=processor.padding_av)
    else:
