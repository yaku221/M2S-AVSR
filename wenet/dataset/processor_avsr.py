...
def load_video_by_key(
    ...
    """ Load lips by key
        ...
        Args:
            ...
            video_data_list_file: video data list
            dataset_name: load dataset name, eg. 'misp2021'...

        Returns:
            {key, wav, txt, lips, label, sample_rate}
    """
    # import ipdb; ipdb.set_trace()
    try:
        assert 'key' in sample
        assert 'wav' in sample
        assert 'txt' in sample        
        key = sample['key']  # 'S132_R04_S132133_C09_I2_Far_4_109940-110056'   'R67_S395396397398_C03_I1_Middle-398-083716_084172.flac'
        waveform = sample['wav']  #  (1, X), tensor([[-0.0027, -0.0029, -0.0042,  ...,  0.0057,  0.0049,  0.0036]])
        sample_rate = sample['sample_rate']
        txt = sample['txt']        

        ### use video_load api
        # print(f"dataset_name: {dataset_name}")
        if dataset_name == "misp2021":
            spk_session_id, start, end = parse_misp2021_data(key)
        elif dataset_name == "realav1":
            spk_session_id, start, end = parse_realav1_data(key, max_speakers)  # return base spk_session_id, no D0-D2
        elif dataset_name == "lrs3":
            spk_session_id, start, end = key, None, None
        elif dataset_name == "general_data":
            spk_session_id, start, end = parse_general_data(key)
        else:
            raise ValueError(f"Unsupported data_type: {dataset_name}")
        # import ipdb; ipdb.set_trace()
        ### spk_session_id  ==> video_key ==> video_path
        view_dict = video_key2lips.get(spk_session_id, {}) if video_key2lips is not None else {}
        
        def _load_one_video(vpath: str):
            # import ipdb; ipdb.set_trace()
            if vpath is None:
                return None
            if not os.path.exists(vpath):
                logging.warning(f"Video file not exists: {vpath}")
                return None
            suffix = os.path.splitext(vpath)[1].lower()
             # load avi/mp4 or pt or pkl files
            if suffix == '.pt':
                # # import ipdb; ipdb.set_trace()
                # load .pt visual files
                # NOTE: use tools/avi2pt.py convert avi video files to pt video files,
                # have resize to (T, 96, 96, 3(BGR)) and normalized, 
                # data_type=torch.float32, between [0.0, 1.0] 
                            
                # lips_frames = torch.load(vpath).to(dtype=torch.float32)
                lips_frames = torch.load(vpath, map_location="cpu", weights_only=True).to(dtype=torch.float32)
                if video_aug:
                    return augment_pt_video(
                        lips_frames, 
                        train=train,
            ...
            # load .pkl visual files
            elif suffix == '.pkl':                
                with open(vpath, "rb") as f:
                    encoded_frames = pickle.load(f)
                    if not encoded_frames:
                        logging.warning(f"Empty pkl file: {vpath}")
                        return None
                    first = cv2.imdecode(np.frombuffer(encoded_frames[0], np.uint8), cv2.IMREAD_COLOR)
                    h, w, _ = first.shape
                    lips_frames = [cv2.imdecode(np.frombuffer(fr, np.uint8), cv2.IMREAD_COLOR) for fr in encoded_frames]
                    lips_frames = np.stack(lips_frames, axis=0)
                    if h != video_res or w != video_res:
                        lips_frames = resize_video(lips_frames, video_res, video_res)
                    if video_aug:                        
                        aug = augment_frames_numpy(
                            lips_frames, 
                            train=train,
                            image_crop_size=image_crop_size,
                            image_mean=image_mean, 
                        ...
                        return torch.from_numpy(np.ascontiguousarray(aug)).to(dtype=torch.float32)
                    return torch.from_numpy(np.ascontiguousarray(lips_frames)).to(dtype=torch.float32) / 255.0
            
            # load .avi/mp4 video files
            elif suffix == '.avi' or suffix == '.mp4': 
                # import ipdb; ipdb.set_trace()   
                if start is None or end is None:
                    lips_frames, fps = load_video(vpath)
                else:
                    lips_frames, fps = load_video(vpath, (start, end))
                # lips_frames.shape: (num_frames, height, width, channels)    
                # print(f"lips_frames.shape: {lips_frames.shape}")
                # import ipdb; ipdb.set_trace()
                if len(lips_frames) == 0:
                    raise ValueError(f"No frames in {vpath}")
                lips_frames = resize_video(lips_frames, video_res, video_res)                
                if video_aug:
                    # import ipdb; ipdb.set_trace()
                    aug = augment_frames_numpy(
                    ...
                    return torch.from_numpy(aug)
                return torch.from_numpy(np.ascontiguousarray(lips_frames)).to(dtype=torch.float32) / 255.0

        # # ---- one direction ----
        # if directions == "1d":
        #     vpath = view_dict.get("D1", None)  
        #     sample["video"] = _load_one_video(vpath)
        #     # import ipdb; ipdb.set_trace()
        #     return sample
        ...
        #     for vv in ("D0", "D1", "D2"):
        #         ns = dict(sample)
        #         ns["key"] = f"{sample['key']}_{vv}"
        #         ns["video"] = _load_one_video(view_dict.get(vv, None))
        #         outs.append(ns)
        #     # import ipdb; ipdb.set_trace()
        #     return outs

        # ---- single direction: d0/d1/d2 -> D0/D1/D2 ----
        if directions in ("d0", "d1", "d2"):
            vv = directions.upper()  # "d0" -> "D0"
            vpath = view_dict.get(vv, None)
            sample["video"] = _load_one_video(vpath)  
            return sample
        ...
        k = sample.get('key', 'Unknown key')
        logging.warning(f"{k} failed to load video: {ex}")
        raise
...
def tokenize(sample, tokenizer: BaseTokenizer):
    ...
    """
    assert 'txt' in sample
    tokens, label = tokenizer.tokenize(sample['txt'])  # text ==> tokens, ids = "b'AA==", 188
...
def padding_av(data):
    """ Padding the data into training data
        ...
        Returns:
            Tuple(keys, feats, labels, feats lengths, label lengths)
    """
    # print(f"first feat shape = {tuple(data[0]['feat'].shape)}")
    # print(f"first video shape = {tuple(data[0]['video'].shape)}")
    # print(f"first label len = {len(data[0]['label'])}")
    # import ipdb; ipdb.set_trace()
    sample = data
    assert isinstance(sample, list)
    ...
    feats_a_lengths = torch.tensor([sample[i]['feat'].size(0) for i in order],
                                 dtype=torch.int32)
    feats_v_lengths = torch.tensor([sample[i]['video'].size(0) for i in order],
                                 dtype=torch.int32)
    sorted_feats_a = [sample[i]['feat'] for i in order]

    v0 = sample[order[0]]['video']  # (T0, H, W, C)
    assert v0.dim() == 4, f"video must be (T,H,W,C), got {tuple(v0.shape)}"
    _, H, W, C = v0.shape
    sorted_feats_v = [
        sample[i]['video'].reshape(sample[i]['video'].size(0), H * W * C)
        for i in order
    ]    
    ...
    padded_feats_a = pad_sequence(sorted_feats_a,
                                    batch_first=True,
                                    padding_value=0)
    padding_feats_v = pad_sequence(sorted_feats_v,
                                batch_first=True,
                                padding_value=0)
    
    B, T = padding_feats_v.shape[0], padding_feats_v.shape[1]
    padded_feats_v = padding_feats_v.view(B, T, H, W, C)
    ...
    # import ipdb; ipdb.set_trace()        
    # === A/V ratio monitor for debug ===
    # try:
    #     a_len = feats_a_lengths.float()
    #     v_len = feats_v_lengths.clamp(min=1).float()
    #     av_ratio = a_len / v_len
    #     print(f"A/V ratio mean={av_ratio.mean().item():.2f} "
    #         f"std={av_ratio.std().item():.2f} "
    #         f"min={av_ratio.min().item():.2f} max={av_ratio.max().item():.2f}")
    # except Exception as e:
    #     print(f"A/V ratio check failed: {e}")


    batch = {
        "keys": sorted_keys,
        "feats": padded_feats_a,
        "video": padded_feats_v,
        "target": padding_labels,
        "feats_lengths": feats_a_lengths,
        "video_lengths": feats_v_lengths,
        "target_lengths": label_lengths,
        "pcm": padded_wavs,
        "pcm_length": wav_lengths,
        "langs": langs,
