...
from wenet.utils.scheduler_avsr import WarmupLR, NoamHoldAnnealing, CosineAnnealing, WarmupLinearDecay
from wenet.utils.ctc_utils import get_blank_id
from wenet.utils.common import TORCH_NPU_AVAILABLE
from wenet.utils.init_dataset_avsr import init_dataset_avsr
...
def add_dataset_args(parser):
    ...
    parser.add_argument('--train_audio_data', required=True, help='train audio data file')
    parser.add_argument('--train_video_data', required=True, help='train video data file')
    parser.add_argument('--cv_audio_data', required=True, help='cv audio data file')
    parser.add_argument('--cv_video_data', required=True, help='cv video data file')
    parser.add_argument('--test_audio_data', help='test audio data file')
    parser.add_argument('--test_video_data', help='test video data file')
...
def check_modify_and_save_config(args, configs, symbol_table):
    ...
    configs['output_dim'] = configs['vocab_size']  # from tokenizer.vocab_size()
...
def init_dataset_and_dataloader_avsr(args, configs, tokenizer, seed=777):
    ...
    dataset_type = configs.get('dataset', 'avsr')
    configs['vocab_size'] = tokenizer.vocab_size()
    train_dataset = init_dataset_avsr(dataset_type,
                                 args.data_type,
                                 args.train_audio_data,
                                 args.train_video_data,
                                 tokenizer,
                                 conf,
                                 partition=True,
                                 split='train')
    cv_dataset = init_dataset_avsr(dataset_type,
                              args.data_type,
                              args.cv_audio_data,
                              args.cv_video_data,
                              tokenizer,
                              ...
                              partition=False,
                              split='cv')
    if configs['val_test_data']:
       test_dataset = init_dataset_avsr(dataset_type,
                              args.data_type,
                              args.test_audio_data,
                              args.test_video_data,
                              tokenizer,
                              conf,
                              partition=False,
                              split='test') 
    ...
    if configs['val_test_data']:
       return train_dataset, cv_dataset, test_dataset, train_data_loader, cv_data_loader, test_data_loader
...
def wrap_cuda_model(args, model, configs=None):
    ...
    if args.train_engine == "torch_ddp":  # native pytorch ddp
        ...
        if configs is not None:
            ctc_weight = float(configs.get('model_conf', {}).get('ctc_weight', 0.0))            
        find_unused = (grad_ckpt or ctc_weight == 0.0)
...
def _collect_params_by_keywords(model, keywords: List[str]):
    picked, ids = [], set()
...
def init_optimizer_and_scheduler_avsr(args, configs, model):
    groups = []
    lr_groups = configs['optim_conf'].get('lr_groups', None)
    
    # 1. optim params fuzzy matching:
    ## set optim params by av fusion method
    groups = []
    if lr_groups is not None:
        used_ids = set()
        normal_groups = [g for g in lr_groups if g.get('keywords') != ["others"]]
        others_groups = [g for g in lr_groups if g.get('keywords') == ["others"]]

        def match_and_append(g):
            kw = g['keywords']
            lr_value = float(g['lr'])
            if kw == ["others"]:
                matched = [p for n, p in model.named_parameters() if id(p) not in used_ids]
            else:
                matched = [p for n, p in model.named_parameters() if any(k in n for k in kw) and id(p) not in used_ids]
            if len(matched) == 0:
                print(f"lr_group keywords {kw} matched 0 params.")
            groups.append({'params': matched, 'lr': lr_value})
            used_ids.update(id(p) for p in matched)

        for g in normal_groups:
            match_and_append(g)
        for g in others_groups:  # after normal_groups
            match_and_append(g)

        print("Optimizing params (from lr_groups):")
        for g in groups:
            print(f"lr={g['lr']}, num_params={len(g['params'])}")
    
    # 2. optim params exact matching:
    # set optim_conf.modules in different lr, lr[-1] for the rest of params when lr is a list,
    # e.g.:
    # optim_conf:
    # lr: [0.001, 0.0005, 0.0001]
    # modules: ["encoder.encoders.0.xxx", "decoder.decoders.0.xxx"]
    # scheduler: warmuplr
    # if isinstance(lr, List) and len(groups) == 0:
    ...
    elif configs['scheduler'] == 'cosineannealing':
        scheduler_type = CosineAnnealing
        # need: warmup_steps, max_steps
        scheduler = CosineAnnealing(optimizer, **configs['scheduler_conf'])
    elif configs['scheduler'] == 'warmuplineardecay':
        scheduler_type = WarmupLinearDecay
        # need: warmup_steps, max_steps
        scheduler = WarmupLinearDecay(optimizer, **configs['scheduler_conf'])
    else:
...
def batch_forward(model, batch, scaler, info_dict, device):
    ...
    tag = str(info_dict.get("tag", "")).upper()
    track_norm = (tag in {"CV", "TEST"}) and info_dict.get("track_norm", True)
    # autocast context
    ...
    if "cuda" in str(device):
        autocast_context = lambda **kwargs: torch.amp.autocast(
            "cuda", **kwargs
        )
    elif "npu" in str(device) and TORCH_NPU_AVAILABLE:
        autocast_context = lambda **kwargs: torch.amp.autocast(
            "npu", **kwargs
        )
    else:
    ...
    with autocast:
        # import ipdb; ipdb.set_trace()
        loss_dict = model(batch, device, track_norm=track_norm)
...
def log_per_step(writer, info_dict, timer: Optional[StepTimer] = None):
    ...
    # val Tensorboard ((CV / TEST))
    elif "step_" in tag and rank == 0 and writer is not None:
        prefix = info_dict.get('eval_prefix', 'cv')
        for name, value in loss_dict.items():
            writer.add_scalar('{}/{}'.format(prefix, name),
                              tensor_to_scalar(value), step)
        logging.info(
            'Epoch {} Step {} {} info lr {} {}_loss {} rank {} acc {}'.format(
                epoch, step + 1, prefix.upper(), lrs_to_str(lrs),
                prefix, tensor_to_scalar(loss_dict["loss"]), rank,
                tensor_to_scalar(loss_dict["acc"])))
        return

    # TRAIN & VAL, Shell log (stdout)
    if (batch_idx + 1) % log_interval == 0:
        log_str = '{} | '.format(tag)
        if timer is not None:
            timer_step = info_dict.get('cv_step', info_dict.get('test_step', step))
...
def log_per_epoch(writer, info_dict):
    ...
    # Optional test block
    test_loss_dict = info_dict.get("test_loss_dict", None)
    if test_loss_dict is not None:
        logging.info(
            'Epoch {} Step {} TEST info lr {} test_loss {} rank {} acc {}'.format(
                epoch, step, lrs_to_str(lrs), tensor_to_scalar(test_loss_dict["loss"]),
                rank, tensor_to_scalar(test_loss_dict["acc"])))
    ...
    if int(os.environ.get('RANK', 0)) == 0:
        ...
        if test_loss_dict is not None:
            for name, value in test_loss_dict.items():
                writer.add_scalar('test/{}'.format(name), tensor_to_scalar(value), epoch)
