...
class WarmupLinearDecay(_LRScheduler):
    """
   ...
   `HuggingFace Transformers: 
    transformers.get_linear_schedule_with_warmup:
      if step < warmup_steps:
          ...
          lr = base_lr * max(0, (max_steps - step) / (max_steps - warmup_steps))

    Args:
        optimizer: torch.optim.Optimizer
        warmup_steps (int): Number of training steps in warmup stage
        max_steps (int): Total number of training steps        
        last_epoch (int): _LRScheduler: init: -1, step()=0
    """
    def __init__(
        self,
        optimizer,
        *,
        ...
        assert max_steps >= warmup_steps, "max_steps must be >= warmup_steps"
    ...
    def get_lr(self):
        ...
        if step < self.warmup_steps:
            ...
            factor = max(
                0.0,
                float(self.max_steps - step) / float(max(1, self.max_steps - self.warmup_steps))
            )
