...
from wenet.utils.init_model_avsr import init_model_avsr
...
def get_args():
    ...
    parser.add_argument('--test_audio_data', required=True, help='test audio data file')
    parser.add_argument('--test_video_data', required=True, help='test video data file')
    ...
    parser.add_argument('--export_full_ckpt_path',
                        type=str,
                        default='',
                        help='export full state_dict to the path')
    ...
    parser.add_argument('--noise_snr_test',
                        type=float,
                        default=1.0,
                        help='Signal to noise ratio with added noise.')
    parser.add_argument("--noise_fn_test",
                        default=None,
                        type=str,
                        help="The paths of noise wav files.")  
...
def main():
    ...
    # print("[DEBUG] final dataset_conf.max_speakers =", configs["dataset_conf"]["max_speakers"], flush=True)
    # print("[DEBUG] final dataset_conf.directions =", configs["dataset_conf"]["directions"], flush=True)

    if args.add_noise:
        logging.info(f"Decoding noise wavs with SNR: {args.noise_snr_test}.")
        test_conf['add_noise'] = True
        test_conf['train'] = False
        test_conf['split'] = 'test'
        test_conf['noise_prob'] = args.noise_prob
        test_conf['noise_snr_test'] = args.noise_snr_test
        test_conf['noise_fn_test'] = args.noise_fn_test
    else:
        logging.info(f"Decoding wavs without adding noise.")
        test_conf['add_noise'] = False

    test_conf['filter_conf']['max_length'] = 3000
    test_conf['filter_conf']['min_length'] = 0
    test_conf['filter_conf']['token_max_length'] = 400
    ...
    test_conf['video_aug'] = True  # CenterCrop, Normalize 
    test_conf['train'] = False
    ...
    args.jit = False
    model, configs = init_model_avsr(args, configs)

    # export full ckpt
    if args.export_full_ckpt_path:
        export_path = args.export_full_ckpt_path
        os.makedirs(os.path.dirname(export_path), exist_ok=True)
        full_sd = {k: v.detach().cpu() for k, v in model.state_dict().items()}
        torch.save(full_sd, export_path)
        logging.info(f"Exported full ckpt to: {export_path} "
