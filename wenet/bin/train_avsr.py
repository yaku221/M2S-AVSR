...
from wenet.utils.executor_avsr import Executor
from wenet.utils.config import override_config
from wenet.utils.init_model_avsr import init_model_avsr
from wenet.utils.init_tokenizer import init_tokenizer
from wenet.utils.train_utils_avsr import (
    add_fsdp_args, add_model_args, add_dataset_args, add_ddp_args,
    add_deepspeed_args, add_trace_args, init_distributed,
    init_dataset_and_dataloader_avsr, check_modify_and_save_config,
    init_optimizer_and_scheduler_avsr, init_scaler, trace_and_print_model,
...
def main():
    ...
    # validate test set
    val_test_enabled = configs.get('val_test', False)
    has_test_data = getattr(args, 'test_audio_data', None) is not None
    configs['val_test_data'] = bool(val_test_enabled and has_test_data)
    
    # Get dataset & dataloader
    if configs['val_test_data']:
        train_dataset, cv_dataset, test_dataset, train_data_loader, cv_data_loader, test_data_loader = \
            init_dataset_and_dataloader_avsr(args, configs, tokenizer)
    else:
    ...
    model, optimizer, scheduler = init_optimizer_and_scheduler_avsr(
        args, configs, model)
    ...
    for epoch in range(start_epoch, end_epoch):
        ...
        # import ipdb; ipdb.set_trace()
        loss_dict = executor.cv(model, cv_data_loader, configs, writer, tag='CV')
        test_loss_dict = executor.cv(model, test_data_loader, configs, writer, tag='TEST') \
            if configs.get('val_test_data', False) else None
